...
```{r}
# Directory paths
dataDir <- file.path(params$outputDir, "data",Sys.Date(),"neurons_FMR1_WT_KO")
resultsDir <- file.path(params$outputDir, "results",Sys.Date(),"neurons_FMR1_WT_KO")
...
```{r}
selected_samples <- colData(se_dge) %>% as.data.frame() %>% tibble::rownames_to_column(var = "sample_id") %>% dplyr::filter(class == "FMR1_WT_neuron" | class == "FMR1_KO_neuron") %>%
   dplyr::select(-c(batch,phenotype))
...
We work with the dataset: neurons-FMR1 WT/KO.

```{r}
if (file.exists(file.path(dataDir,paste0("dds-neurons_FMR1_WT_KO-",Sys.Date(),".rds")))) {
    dds <- readRDS(file.path(dataDir,paste0("dds-neurons_FMR1_WT_KO-",Sys.Date(),".rds")))
} else {
dds = DESeqDataSetFromMatrix(countData = counts_se_dge_selected, colData = summarydata_se_dge_selected, design = params$design)
dds = DESeq(dds)
saveRDS(dds,file = file.path(dataDir,paste0("dds-neurons_FMR1_WT_KO-",Sys.Date(),".rds")))
}
```
