---
title: "Differential Expression Stem Cells Trisomy21 & Euploid"
...
```{r}
# Directory paths
dataDir <- file.path(params$outputDir, "data",Sys.Date(),"stemcells_Trisomy21_euploid")
resultsDir <- file.path(params$outputDir, "results",Sys.Date(),"stemcells_Trisomy21_euploid")
...
```{r}
selected_samples <- colData(se_dge) %>% as.data.frame() %>% tibble::rownames_to_column(var = "sample_id") %>% dplyr::filter(class == "Trisomy21_stemcell" | class == "Euploid_stemcell" ) %>% dplyr::select(-c(batch,phenotype))
...
if (file.exists(file.path(dataDir,paste0("dds-",Sys.Date(),".rds")))) {
    dds <- readRDS(file.path(dataDir,paste0("dds-stemcells_Trisomy21_euploid-",Sys.Date(),".rds")))
} else {
dds = DESeqDataSetFromMatrix(countData = counts_se_dge_selected, colData = summarydata_se_dge_selected, design = params$design)
dds = DESeq(dds)
saveRDS(dds,file = file.path(dataDir,paste0("dds-stemcells_Trisomy21_euploid-",Sys.Date(),".rds")))
}
```
